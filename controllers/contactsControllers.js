...
const updateContactById = async (req, res, next) => {
    ...
    try {
        ...
        updatedContact ? res.status(200).json(updateContact) : res.status(404).json({ message: "Not found" });
    } catch (error) {
        console.log(error);
        next(error);
    }
...
const deleteContactById = async (req, res, next) => {
    ...
    try {
        const isContactExist = await removeContact(id);
            ...
            isContactExist ?res.status(200).json({ message: "Contact deleted" }) : res.status(404).json({ message: "Not found" });
    } catch (error) {
        console.log(error);
        next(error);
    }
